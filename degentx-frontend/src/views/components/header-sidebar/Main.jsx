...
function HeaderSidebar({ slot, mode }) {
  let navigate = useNavigate();

  const headerStore = useContext(HeaderStoreContext);

  let navbarItems =
    mode == "docs" ? DocsConfig?.navbar?.items : FrontendConfig?.navbar?.items;
          ...
          <div className="flex flex-row">
            ...
            <div>
              <Link to="/" className="intro-x flex items-center ">
                ...
                />
                <span className="side-nav__header__text text-black pt-0.5 text-lg ml-2.5"></span>
              </Link>
            </div>
